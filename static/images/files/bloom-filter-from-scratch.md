...
math: true
cover:
    image: "images/bloom-filter-cover.png"
...
1. **The "Pukka No" (Definite No):** A customer asks for a rare brand of tea. You check the card — the **Red** stamp is missing. You know for a fact it's not there, without even walking to the back. *"Nahi hai, Bhaiyya."* You just saved 10 minutes of pointless searching.
...
![Bloom Filter false positive diagram — Panel A shows bits set by adding "Amul Ghee"; Panel B shows the same three bits set by three different items causing a false positive for "Saffola Oil"](images/bloom_filter_false_positives.svg)
...
Building algorithms like this from scratch is the best way to move from "using libraries" to "understanding systems." Once you know what's inside Guava's `BloomFilter`, you know when to reach for it — and when to do something it can't.
